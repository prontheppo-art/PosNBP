...
<body class="bg-slate-50 text-slate-800 antialiased">
    ...
    <div id="categoryFoodModalOverlay" class="custom-modal-overlay">
        <div class="custom-modal-box" onclick="event.stopPropagation()">
            ...
            <div id="foodButtonsGrid" class="grid grid-cols-3 gap-2 mb-6"></div>

            <button type="button" onclick="closeCategoryFoodModal()" class="w-full bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold py-3 rounded-2xl text-xs transition">
                ปิดหน้านี้
            </button>
        </div>
    </div>
    ...
    <script>
        ...
        async function loadData() {
            ...
            if (!db) {
                container.innerHTML = '<div class="col-span-2 text-center py-8 text-rose-500 font-bold text-xs">❌ ไม่พบการเชื่อมต่อ Supabase</div>';
                return;
            }
            ...
            try {
                ...
                categories = (catRes.data || []).sort((a, b) => Number(a.id) - Number(b.id)); 
                foods = foodRes.data || [];

                renderQuickNav();
            } catch (err) { 
                console.error('Error loading data:', err); 
                container.innerHTML = `<div class="col-span-2 text-center py-8 text-rose-500 font-bold text-xs">❌ เกิดข้อผิดพลาด: ${err.message || 'ไม่สามารถดึงข้อมูลได้'}</div>`;
            }
        }
        ...
        function renderQuickNav() {
            ...
            categories.forEach((cat, index) => {
                ...
                const catFoodIds = foods
                    .filter(f => Number(f.group_id) === Number(cat.id))
                    .map(f => String(f.id));

                const catTotalQty = currentCart
                    .filter(item => catFoodIds.includes(String(item.food_id)))
                    .reduce((sum, item) => sum + (Number(item.qty) || 0), 0);

                let badgeHTML = '';
                if (catTotalQty > 0) {
                    badgeHTML = `<span class="cat-badge-main">${catTotalQty}</span>`;
                }
        ...
        function renderFoodGrid() {
            ...
            if (catFoods.length === 0) {
                gridContainer.innerHTML = '<div class="col-span-3 text-center py-6 text-slate-400 font-bold text-xs">ไม่มีรายการอาหารในหมวดนี้</div>';
            } else {
                catFoods.forEach(food => {
                    const btn = document.createElement('button');
                    btn.type = 'button';
                    btn.className = 'relative border-2 border-emerald-400 text-emerald-800 font-bold p-2 rounded-2xl text-xs flex items-center justify-center text-center bg-white shadow-sm active:bg-emerald-500 active:text-white transition h-16';

                    const foodTotalQty = currentCart
                        .filter(item => String(item.food_id) === String(food.id))
                        .reduce((sum, item) => sum + (Number(item.qty) || 0), 0);

                    let badgeHTML = '';
                    if (foodTotalQty > 0) {
                        badgeHTML = `<span class="food-badge-main">${foodTotalQty}</span>`;
                    }

                    btn.innerHTML = `<span>${food.name}</span>${badgeHTML}`;
                    btn.onclick = () => openOrderDetailModal(food);
        ...
        function confirmClearCart() {
            document.getElementById('confirmClearModalOverlay').classList.add('active');
        }

        function closeConfirmClearModal() {
            document.getElementById('confirmClearModalOverlay').classList.remove('active');
        }
