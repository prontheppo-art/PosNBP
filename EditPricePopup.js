/**
 * EditPricePopup.js
 * จัดการ Pop-up ตารางแก้ไขราคาอาหารประจำหมวดหมู่
 */
    ...
    open({ category, foods, icon, onPriceUpdate, onChangeCategory }) {
        ...
        if (foods.length === 0) {
            ...
            foods.forEach((m, index) => {
                ...
                tr.innerHTML = `
                    <td class="p-2 font-bold text-gray-800 text-left pl-2.5 text-xs cursor-pointer hover:bg-blue-50 active:bg-blue-100 transition rounded-lg" style="width: 49%;" title="แตะเพื่อย้ายกลุ่มอาหาร">
                ...
                // แตะชื่อรายการเพื่อเปลี่ยนกลุ่ม
                const nameTd = tr.querySelector('td');
                nameTd.onclick = () => {
                    if (this.onChangeCatCallback) this.onChangeCatCallback(m.id);
                };
